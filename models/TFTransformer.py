...
class TFT(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_variable_selection = VariableSelectionNetwork(config['embedding_dim'],
                                (config['time_varying_real_variables_encoder'] +  config['time_varying_categorical_variables']),
                                self.hidden_size,
                                self.dropout,
    ...
    def apply_embedding(self, x, static_embedding, apply_masking):
        ...
        embeddings = torch.cat([static_embedding,time_varying_categorical_embedding,time_varying_real_embedding], dim=2)
